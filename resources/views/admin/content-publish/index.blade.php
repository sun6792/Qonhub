...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            <a href="{{ route('admin.content-publish.task', $task->id) }}" class="flex items-center justify-between px-5 py-3 hover:bg-gray-50/60">
                <div class="min-w-0 flex-1">
                    <div class="text-sm font-medium text-gray-800">{{ $task->task_name }}</div>
                    <div class="text-xs text-gray-400">{{ $task->completed_jobs }}/{{ $task->total_jobs }} · {{ $task->created_at->format('m-d H:i') }}</div>
                </div>
        ...
        @else
        <div class="py-12 text-center text-sm text-gray-400">暂无发布任务，请先从内容弹药库提交发布</div>
        @endif
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div class="border-b border-gray-100 px-5 py-3">
            <h2 class="text-sm font-semibold text-gray-800">🌐 全渠道覆盖总览（54个平台）</h2>
        </div>
        <div class="px-5 py-3">
