          ...
          getPlatformInfo(platform.name).then((info) => {
            if (info) {
              chrome.scripting.executeScript({
                ...
                // 页面跳转/Frame销毁时忽略，发布已成功
                if (err.message?.includes("Frame") || err.message?.includes("removed") || err.message?.includes("closed")) {
                  console.log("[MultiPost] Frame removed after publish — expected, ignoring");
                } else {
                  console.error("[MultiPost] Script injection error:", err);
                }
              });
            }
          });
