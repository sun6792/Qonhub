 ...
 * 获取搜狐号账户信息
 */
export async function getSohuAccountInfo(): Promise<AccountInfo | null> {
  try {
    // 访问搜狐号API获取用户信息
    const response = await fetch("https://mp.sohu.com/api/user/info", {
      method: "GET",
      headers: {
    ...
    const result: AccountInfo = {
      ...
      username: userInfo.nickname || userInfo.name || userInfo.username || "搜狐号用户",
      description: userInfo.bio || userInfo.description || "",
      profileUrl: "https://mp.sohu.com/",
    ...
    console.error("获取搜狐号账户信息失败:", error);
    ...
    try {
      ...
      if (usernameElement) {
        const result: AccountInfo = {
          provider: "sohu",
          accountId: "unknown",
          username: usernameElement.textContent || "搜狐号用户",
          description: "",
          profileUrl: "https://mp.sohu.com/",
      ...
      console.error("从页面获取搜狐号信息也失败:", pageError);
    }
