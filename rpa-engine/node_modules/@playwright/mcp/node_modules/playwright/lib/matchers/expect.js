...
function toHaveAccessibleErrorMessage(locator, expected, options) {
  return toMatchText.call(this, "toHaveAccessibleErrorMessage", locator, "Locator", async (isNot, timeout, signal) => {
    const expectedText = serializeExpectedTextValues([expected], { ignoreCase: options?.ignoreCase, normalizeWhiteSpace: true });
    return await locator._expect("to.have.accessible.error.message", { expectedText, isNot, timeout }, signal);
  }, expected, options);
}
...
function toHaveTitle(page, expected, options = {}) {
  return toMatchText.call(this, "toHaveTitle", page, "Page", async (isNot, timeout, signal) => {
    ...
    return await page.mainFrame()._expect("to.have.title", { expectedText, isNot, timeout }, signal);
  }, expected, options);
}
function toHaveURL(page, expected, options) {
  ...
  return toMatchText.call(this, "toHaveURL", page, "Page", async (isNot, timeout, signal) => {
    const expectedText = serializeExpectedTextValues([expected], { ignoreCase: options?.ignoreCase });
    return await page.mainFrame()._expect("to.have.url", { expectedText, isNot, timeout }, signal);
  }, expected, options);
}
...
` + expected;
  ...
  const { matches: pass, received, log, timedOut, errorMessage } = locator ? await locator._expect("to.match.aria", expectParams, options.signal) : await receiver.mainFrame()._expect("to.match.aria", expectParams, options.signal);
