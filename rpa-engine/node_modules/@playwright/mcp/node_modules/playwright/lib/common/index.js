...
var Suite = class _Suite extends Base {
  constructor(title, type) {
    ...
    this._type = type;
    this.skip = wrapFunctionWithLocation((location, reason) => this._modifier("skip", location, reason));
    this.fixme = wrapFunctionWithLocation((location, reason) => this._modifier("fixme", location, reason));
    this.fail = wrapFunctionWithLocation((location, reason) => this._modifier("fail", location, reason));
  }
  get type() {
    return this._type;
  }
  ...
  _modifier(type, location, reason) {
...
var TestCase = class _TestCase extends Base {
  constructor(title, fn, testType, location) {
    ...
    this.skip = wrapFunctionWithLocation((location2, reason) => this._modifier("skip", location2, reason));
    this.fixme = wrapFunctionWithLocation((location2, reason) => this._modifier("fixme", location2, reason));
    this.fail = wrapFunctionWithLocation((location2, reason) => this._modifier("fail", location2, reason));
  }
  titlePath() {
    const titlePath = this.parent ? this.parent.titlePath() : [];
    titlePath.push(this.title);
