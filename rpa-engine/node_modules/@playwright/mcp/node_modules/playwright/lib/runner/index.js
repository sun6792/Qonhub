...
var Multiplexer = class {
  ...
  async preprocessSuite(config2, suite) {
    const shardingReporters = [];
    for (const reporter of this._reporters) {
      const result = await reporter.preprocessSuite?.(config2, suite);
...
var ReporterV2Wrapper = class {
  ...
  async preprocessSuite(config2, suite) {
    return await this._reporter.preprocessSuite?.(config2, suite);
  }
  onBegin(suite) {
...
async function createRootSuite(testRun, errors, shouldFilterOnly) {
  ...
  const preprocessResult = await testRun.reporter.preprocessSuite(config2.config, rootSuite);
  if (config2.config.shard && !preprocessResult?.implementsSharding) {
    const testGroups = [];
