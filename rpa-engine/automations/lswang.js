/**
 * 领商网 (lswgmt.net) 企业注册认证脚本。
 *
 * 领商网是免费B2B产品推广平台，支持企业注册开店。
...
class LsScript extends BasePlatformScript {
    registerUrl = "https://www.lswgmt.net/register/";
    certifyUrl = "https://www.lswgmt.net/member/";
    ...
    async registerFlow(page) {
        ...
        await page.goto("https://www.lswgmt.net/", { waitUntil: "domcontentloaded", timeout: 15000 });
        ...
        const regEntry = await this.findElement(page, [
            ...
            'a:has-text("免费加入")', 'a:has-text("会员注册")',
        ]);
        if (regEntry) {
        ...
        if (!body1.includes("注册") || body1.length < 200) {
            const regCandidates = [
                "https://www.lswgmt.net/user/register/",
                "https://www.lswgmt.net/member/register/",
                "https://www.lswgmt.net/register/",
                "https://member.lswgmt.net/register/",
            ];
        ...
        } else if (body2.includes("注册成功") || body2.includes("成功提交") || body2.includes("欢迎") || body2.includes("会员")) {
        ...
        const memberUrls = [
            "https://www.lswgmt.net/member/",
            "https://www.lswgmt.net/user/",
            "https://www.lswgmt.net/member/index.php",
            "https://www.lswgmt.net/my/",
        ];
        let loggedIn = false;
        for (const url of memberUrls) {
            try {
                ...
                if (body.includes("会员") || body.includes("企业") || body.includes("个人中心") || body.includes("我")) {
                    loggedIn = true;
        ...
        if (!loggedIn) {
            ...
            const loginUrls = [
                "https://www.lswgmt.net/user/login/",
                "https://www.lswgmt.net/member/login/",
                "https://www.lswgmt.net/login/",
            ];
        ...
        const editCandidates = [
            "https://www.lswgmt.net/member/company/",
            "https://www.lswgmt.net/member/edit/",
            "https://www.lswgmt.net/member/info/",
            "https://www.lswgmt.net/member/company/edit/",
            "https://www.lswgmt.net/user/info/",
        ];
        let editFound = false;
        ...
        if (!editFound) {
            try {
                await page.goto("https://www.lswgmt.net/member/", { waitUntil: "domcontentloaded", timeout: 10000 });
                await this.wait(2000, 3000);
            } catch {}
        ...
        for (const link of allLinks) {
            try {
                ...
                if (href && (
                    ...
                    const candidate = href.startsWith("http") ? href : `https://www.lswgmt.net${href.startsWith("/") ? "" : "/"}${href}`;
        ...
        if (!shopUrl) {
            ...
            // 尝试子域名和路径两种格式
            const possibleUrls = [
                `https://${companySlug}.lswgmt.net/`,
                `https://www.lswgmt.net/shop/${companySlug}/`,
                `https://www.lswgmt.net/company/${companySlug}/`,
            ];
            ...
            if (!shopUrl) {
                shopUrl = `https://www.lswgmt.net/shop/${companySlug}/`;
            }
...
export async function execute(opts) {
    const script = new LsScript(opts.taskId, opts.account, opts.enterprise, opts.options, opts.logger.info.bind(opts.logger));
