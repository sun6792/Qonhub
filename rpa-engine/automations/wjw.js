/**
 * 全球五金网 (wjw.cn) 企业注册认证脚本。
 *
 * 全球五金网是中国最大的五金机电行业B2B平台，拥有大量采购商资源。
...
export const platform = "wjw";
export const description = "全球五金网 — 五金机电行业B2B平台注册";

class WjwScript extends BasePlatformScript {
    registerUrl = "https://www.wjw.cn/register/";
    certifyUrl = "https://www.wjw.cn/member/";
    ...
    async registerFlow(page) {
        ...
        await page.goto("https://www.wjw.cn/", { waitUntil: "domcontentloaded", timeout: 15000 });
        ...
        const regEntry = await this.findElement(page, [
            ...
            'a:has-text("免费入驻")', '.reg-btn', '.register-btn', 'a:has-text("会员注册")',
        ]);
        if (regEntry) {
        ...
        if (!body1.includes("注册") || body1.length < 200) {
            const regCandidates = [
                "https://www.wjw.cn/user/register/",
                "https://www.wjw.cn/member/register/",
                "https://www.wjw.cn/register/",
                "https://member.wjw.cn/register/",
                "https://www.wjw.cn/reg/",
            ];
        ...
        if (body2.includes("已被注册") || body2.includes("已存在") || body2.includes("已注册")) {
            ...
            this.log("  Verification needed (SMS likely required) — screenshot captured");
        ...
        const memberUrls = [
            "https://www.wjw.cn/member/",
            "https://www.wjw.cn/user/",
            "https://www.wjw.cn/member/index.php",
            "https://www.wjw.cn/my/",
            "https://my.wjw.cn/",
        ];
        let loggedIn = false;
        ...
        if (!loggedIn) {
            ...
            const loginUrls = [
                "https://www.wjw.cn/user/login/",
                "https://www.wjw.cn/member/login/",
                "https://www.wjw.cn/login/",
                "https://passport.wjw.cn/login/",
            ];
        ...
        const editCandidates = [
            "https://www.wjw.cn/member/company/",
            "https://www.wjw.cn/member/edit/",
            "https://www.wjw.cn/member/info/",
            "https://www.wjw.cn/member/company/edit/",
            "https://www.wjw.cn/user/info/",
            "https://www.wjw.cn/member/shop/",
        ];
        let editFound = false;
        ...
        if (!editFound) {
            try {
                await page.goto("https://www.wjw.cn/member/", { waitUntil: "domcontentloaded", timeout: 10000 });
                await this.wait(2000, 3000);
            } catch {}
            const editLink = await this.findElement(page, [
                'a:has-text("企业信息")', 'a:has-text("公司资料")', 'a:has-text("基本资料")',
                'a:has-text("编辑")', 'a:has-text("修改")', 'a:has-text("完善")', 'a:has-text("商铺设置")',
            ]);
            if (editLink) {
        ...
        for (const link of allLinks) {
            try {
                ...
                if (href && (
                    ...
                    const candidate = href.startsWith("http") ? href : `https://www.wjw.cn${href.startsWith("/") ? "" : "/"}${href}`;
        ...
        if (!shopUrl) {
            ...
            // 全球五金网常见店铺格式
            const possibleUrls = [
                `https://${companySlug}.wjw.cn/`,
                `https://www.wjw.cn/shop/${companySlug}/`,
                `https://www.wjw.cn/company/${companySlug}/`,
                `https://shop.wjw.cn/${companySlug}/`,
            ];
            ...
            if (!shopUrl) {
                shopUrl = `https://www.wjw.cn/company/${companySlug}/`;
            }
        ...
        return {
            ...
            account_id: username,
            status: "certified",
            message: "全球五金网企业认证已提交",
        };
    }
}

export async function execute(o) {
    const script = new WjwScript(o.taskId, o.account, o.enterprise, o.options, o.logger.info.bind(o.logger));
